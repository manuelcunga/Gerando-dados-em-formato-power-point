...
<head>
	...
	<title>Gerador de POWER POINT</title>
	...
	<style>
		...
		/* -- brentstrap.css ---------- */
		.tab-pane { background-color: var(--white); }
		...
		.tabHtmlToPpt thead th { background: var(--light); color: white; font-weight: normal; }
		.tabHtmlToPpt tbody td { background: var(--light); color: var(--dark); }
		.tabHtmlToPpt th, .tabHtmlToPpt td { border: 1px solid var(--light); padding: 4px 8px; }
	</style>
    ...
    <script>
function gerarPowerPoint(){
			        // STEP 1: Criando nova apresentação power Point
var pptx = new PptxGenJS();

// STEP 2: Adicionando slider a apresentação
var slide = pptx.addSlide();
// STEP 3: adicionando qualquer objetos ao slier
...
slide.addText(
  'Gerando relatorio em power point',
  { x:0.0, y:0.25, w:'100%', h:1.5, align:'center', fontSize:24, color:'0088CC', fill:{ color:'F1F1F1' } }
);
// STEP 4: salvando e download
pptx.writeFile('PptxGenJs-Basic-Slide-Demo');
  ...
  <div class="container">
		<br>
		<button class="btn btn-primary" onclick="gerarPowerPoint();">Gerar</button>
  </div>